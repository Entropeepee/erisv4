...
# Find all maps in the AncientContent folder
all_maps = asset_reg.get_assets_by_path('/Game/AncientContent', recursive=True)
for m in all_maps:
    if m.asset_class_path.package_name == '/Script/Engine' and m.asset_class_path.asset_name == 'World':
        unreal.log(f"Found Ancient Map: {m.get_full_name()}")
"""
